...
-- #2. ((The cat is old) and (The dog is a puppy)) → (The cat is old) or (The dog is a puppy)
def e1 := (A ∧ B) ⇒ (A ∨ B)
-- #3. ((cheese) × (jam)) → no((no jam) ⊕ (no jam))
def e2 := (C ∧ J) ⇒ ¬(¬C ∨ ¬J)
-- #4. ((The cat is old) ∨ (The dog is a puppy)) ∧ ((cheese) ∨ (jam)) ⇒ ((The cat is old) ∧ (cheese)) ∨ ((The dog is a puppy) ∧ (jam))
def e3 := (A ∨ B) ∧ (C ∨ J) ⇒ (A ∧ C) ∨ (B ∧ J)
...
def my_interp : Interp 
| var.mk 0 => true   -- Assign true to variable b
| var.mk 1 => false  -- Assign false to variable j
| var.mk 2 => true   -- Assign true to variable c
| var.mk 3 => false  -- Assign false to variable a
| _ => true   -- Assign true to all other variables (wildcard)
