...
#eval list_rev [1,2,3]    --expect [3,2,1]
#eval list_rev ["a", "b", "c"]    --expect ["c","b","a"]
...
def reduce_or : List Bool → Bool --reduce a list of booleans to find true IF ONLY ONE element true
| [] => false
| h::t => or h (reduce_or t)

def reduce_and' : List Bool → Bool --reduce a list of booleans to find true IF ALL elements are true
| [] => true
| h::t => and h (reduce_and t) 
...
-- ** every element in a set of elements thats empty is any property you want
--    EXAMPLE reduce_and' determines empty list as true
