...
--if you get an error with #eval, try typing #reduce instead
...
#check Box Nat --expect: Type

def box_containing_zero : Box Nat := Box.put 9
...
#check (@Box.put)       -- @put : {α : Type} → α → Box α 
--This is Type → term of Type → Box of the type

-- turn off implicit types by using @
...
| (put o) => o              --Box.put applied to some argument o, then just return o
...
want to *get* the first, or respectively the second, value, "out 
